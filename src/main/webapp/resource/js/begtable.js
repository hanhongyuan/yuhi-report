...
layui.define(['layer', 'laypage', 'icheck'], function(exports) {
	...
	begTable.prototype.getSelectedRows = function() {
		var container=getContainer(this.config);
		var rows=$(container).find("tr");
		var elemAry=[];
		for (var int = 1; int < rows.length; int++) {
			var elem=rows.eq(int).find("input[type=checkbox]")[0];
			if(elem.checked)elemAry.push(elem);
		}
		return elemAry;
	};
	/**
	 * @description 获取选择的行数据
	 */
	begTable.prototype.getSelectedData = function() {
		var rowsdata=this.config.data;
		var container=getContainer(this.config);
		var rows=$(container).find("tr");
		var elemAry=[];
		for (var int = 1; int < rows.length; int++) {
			var elem=rows.eq(int).find("input[type=checkbox]")[0].checked;
			if(elem)elemAry.push(rowsdata[int-1]);
		}
		return elemAry;
	};
	/**
	 * @description 获取选择的行下标
	 */
	begTable.prototype.getSelectedIndexs = function() {
		var container=getContainer(this.config);
		var rows=$(container).find("tr");
		var elemAry=[];
		for (var int = 1; int < rows.length; int++) {
			var elem=rows.eq(int).find("input[type=checkbox]")[0].checked;
			if(elem)elemAry.push(int);
		}
		return elemAry;
	};
	/**
	...
	begTable.prototype.init = function() {
		...
		var $container=getContainer(_config);
		this.regexTable(_config.data);
		//checkbox
		if(_config.checked) {
			//渲染checkbox
...
//	渲染容器数据
	begTable.prototype.regexTable=function(data,curpage,totalpage){
		...
		if(_config.paged) {
			...
			loadData($tableBox,curpage,totalpage,_config);
		} else {
			container.find('.' + ELEM.table).css('margin-bottom', '0px');
		}
	}
	//获取数据容器
	function getContainer(_config){
		var elem = _config.elem;
	 ...
	 * @param {Number} page
	 */
	function loadData($tableBox, page,total,config) {
		total=total?total:config.total;
		laypage({
			...
			jump:function(obj, first) {
				...
				var curr = obj.curr;
				if(!first) {
					if(config.loaddatas)config.loaddatas(obj.curr,total);
