...
layui.define(['element', 'common'], function(exports) {
	...
	function getHtml(data) {
		...
		for(var i = 0; i < data.length; i++) {
			...
			if(data[i].chirdmenus !== undefined && data[i].chirdmenus.length > 0) {
				...
				for(var j = 0; j < data[i].chirdmenus.length; j++) {
					ulHtml += '<dd>';
					ulHtml += '<a href="javascript:;" data-url="' + data[i].chirdmenus[j].url + '">';
					if(data[i].chirdmenus[j].icon !== undefined && data[i].chirdmenus[j].icon !== '') {
						if(data[i].chirdmenus[j].icon.indexOf('fa-') !== -1) {
							ulHtml += '<i class="fa ' + data[i].chirdmenus[j].icon + '" data-icon="' + data[i].chirdmenus[j].icon + '" aria-hidden="true"></i>';
						} else {
							ulHtml += '<i class="layui-icon" data-icon="' + data[i].chirdmenus[j].icon + '">' + data[i].children[j].icon + '</i>';
						}
					}
					ulHtml += '<cite>' + data[i].chirdmenus[j].name + '</cite>';
					ulHtml += '</a>';
					ulHtml += '</dd>';
				}
				ulHtml += '</dl>';
